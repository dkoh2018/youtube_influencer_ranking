...
async function testSentimentAnalysis() {
  ...
  try {
    ...
    // For each influencer, get comments and get top N longest, where N is DEFAULT_CONFIG.commentsPerVideo
...
👤 Processing Influencer ${influencerIndex + 1}/${influencers.length}: ${influencer.username} (${influencer.channel_title})`);
      ...
      console.log(`🎥 Getting ${DEFAULT_CONFIG.videosToScanForComments} most recent videos for ${influencer.username}...`);
      const recentVideos = await db.getInfluencerVideos(influencer.channel_id, DEFAULT_CONFIG.videosToScanForComments);
...
📹 Processing Video ${i + 1}: ${video.title}`);
      ...
      const allComments = await db.getVideoComments(video.videoId, DEFAULT_CONFIG.maxDatabaseQueryLimit);
